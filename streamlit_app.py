...
# ============================================================
# PRIZEPICKS WRAPPER (SESSION + FULL BROWSER HEADERS)
# ============================================================
class PrizePicks:
    BASE = "https://api.prizepicks.com/projections"
    SITE = "https://www.prizepicks.com/"

    @staticmethod
    def _session():
        """
        Build a requests.Session that looks like a real Chrome browser.
        We first 'visit' the main site, then call the projections endpoint.
        """
        s = requests.Session()
        ...
        try:
            # Warm-up visit to main site (may set cookies / CF tokens)
            s.get(PrizePicks.SITE, timeout=10)
        except Exception:
            # If this fails we still return the session; projections call may or may not work.
            pass
    ...
    @staticmethod
    def get_data():
        """Fetch PrizePicks projections (NBA) using a browser-like session."""
        try:
            s = PrizePicks._session()

            api_headers = {
                "User-Agent": s.headers["User-Agent"],
                "Accept": "application/json, text/plain, */*",
                "Accept-Language": "en-US,en;q=0.9",
                "Origin": PrizePicks.SITE.rstrip("/"),
                "Referer": PrizePicks.SITE,
            ...
            if e.response is not None and e.response.status_code == 403:
                st.error(
                    "❌ PrizePicks is returning **403 Forbidden** to this server.\n\n"
                    "That usually means they are blocking cloud/server IPs and "
                    "only allowing real browsers. The app UI will still work, "
                    "but live PrizePicks data can't be loaded from Streamlit Cloud."
                )
            else:
...
def load_projections():
    ...
    if df.empty:
        st.warning(
            "No NBA PrizePicks projections available.\n\n"
            "If you're still seeing a 403 error above, that's PrizePicks "
            "blocking server traffic (Streamlit Cloud). The UI is ready, "
            "but we can't pull live data from here."
        )
    return df
...
# ============================================================
st.caption(
    "NBA Outlier Cloud Engine • Player Card Layout • "
    "If you consistently see 403 from PrizePicks, they are blocking "
    "cloud/server traffic; run locally or via your own proxy for live data."
)
