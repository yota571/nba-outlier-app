...
# SIDEBAR UI
# =====================================================
st.sidebar.title("NBA Outlier-Style App (PrizePicks) – v1.6.1")
...
# =====================================================
st.title("NBA Prop Edge Finder (PrizePicks) – v1.6.1")
st.caption(
...
with tab_games:
    ...
    if games_ml_df.empty:
        ...
        if ml_df.empty:
            ...
            st.markdown("#### All model moneylines & spreads (by team)")
            st.dataframe(
                ml_df[show_cols],
                use_container_width=True,
            )
            ...
            # Favorites per game
            st.markdown("#### Favorites per game (highest model win %)")
            favorites = (
            ...
            # Strong spread edges
            st.markdown(
                f"#### Games with strong model spreads (|spread| ≥ {spread_highlight})"
            )
            strong = favorites[favorites["model_spread"].abs() >= spread_highlight]
            if strong.empty:
                st.info("No games exceed the current model spread threshold.")
            else:
                st.dataframe(
                    strong[fav_cols],
                    use_container_width=True,
