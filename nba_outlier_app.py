...
# - Bet Tracker tab (hit/miss + learning) with CSV persistence
# - "Track this bet" button on player cards (player props)
# - Relaxed status filter + safer "today" filter so more games appear
# - Game Moneylines tab – model win % and ML odds from prop edges
# - Model game spreads + highlight games with strong spreads
# - Game cards above tables, with team logos
# - "Track favorite ML" and "Track favorite spread" for games
# - Game bets saved with bet_category so they can be analyzed later
# - FIX: nba_api team lookup is wrapped in try/except so logos never crash app
# - NEW: Mobile mode – single-column cards & tables in expanders
# =====================================================
...
@st.cache_data
def get_all_teams():
    """Safely fetch NBA teams; return [] if nba_api fails."""
    try:
...
def get_team_logo_url(team_name: str | None) -> str | None:
    """
    Best-effort mapping from team string (name or abbreviation) to NBA logo URL.
    Never throws; returns None if lookup fails.
    """
    if not team_name:
        return None
    ...
    # Try abbreviation exact match
    for t in all_teams:
        if team_name.lower() == t["abbreviation"].lower():
            return f"https://cdn.nba.com/logos/nba/{t['id']}/global/L/logo.png"

    # Try full_name exact match
    for t in all_teams:
        if team_name.lower() == t["full_name"].lower():
            return f"https://cdn.nba.com/logos/nba/{t['id']}/global/L/logo.png"

    # Try nickname (e.g. "Lakers")
    for t in all_teams:
        if team_name.lower() == t["nickname"].lower():
            return f"https://cdn.nba.com/logos/nba/{t['id']}/global/L/logo.png"

    # Try partial match in full_name
    for t in all_teams:
...
# SIDEBAR UI
# =====================================================
st.sidebar.title("NBA Outlier-Style App (PrizePicks) – v1.9.0")
...
st.sidebar.markdown("---")
with st.sidebar.expander("What is Edge / Confidence / Odds / Moneylines / Spreads / Bet side?"):
    st.write(
        ...
        "- **Game Moneylines tab** = uses prop edges (mainly scoring markets) to build a team-level signal, "
        "then converts that to a win probability and moneyline-style odds.\n"
        "- **Model spreads** = same team-level signal, centered within each game and scaled into a point spread.\n"
        "  Positive spread ⇒ model favorite by that many points; negative ⇒ model dog.\n"
...
# quick sanity label for how many props we got from DFS_Wrapper
st.caption(
    f"Data source: DFS_Wrapper PrizePick.get_data(...) "
    f"(rows before filters: {len(props_df)})"
)
...
# =====================================================
st.title("NBA Prop Edge Finder (PrizePicks) – v1.9.0")
st.caption(
    "PrizePicks full-game props vs nba_api stats → edges, confidence %, prediction, model odds, game moneylines & spreads, trends, and bet tracking."
)
...
def build_moneyline_predictions(edges, scoring_markets=None):
    ...
    # ---------------- Spread piece ----------------
    # Center avg_signal per game so they sum to ~0,
    # then scale to points. Positive spread ⇒ model favorite.
...
with tab_cards:
    ...
    if top_n == 0:
        ...
        # Helper: render a single card (so we don't duplicate logic)
        ...
        # Layout: mobile → 1 card per row; desktop → 2 per row
        if mobile_mode:
...
with tab_games:
    ...
    if games_ml_df.empty:
        ...
        if ml_df.empty:
            ...
            # --- Favorites per game (one row per game_key: the favorite) ---
            favorites = (
            ...
            if not fav_list:
                ...
                def render_game_card(fav, k_suffix: str):
                    ...
                    # find the other side (dog) in ml_df
                    dog_row = ml_df[
            ...
            def highlight_ml(row):
                ...
                if wp >= ml_winprob_highlight and props_c >= min_props_for_ml:
                    color = "background-color: #d4f8d4"  # strong favorite
                elif wp < 50:
                    color = "background-color: #f8d4d4"  # our model doesn't like them
                else:
                    color = "background-color: #fff4c2"  # mild edge / coinflip
            ...
            # ========== SPREAD TABLE ==========
            st.markdown(f"#### Model Spreads (favorites per game) – highlight |spread| ≥ {spread_highlight}")
            ...
            def highlight_spread(row):
                ...
                if abs_sp >= spread_highlight and props_c >= min_props_for_ml:
                    color = "background-color: #d4f8d4"  # strong spread edge
                elif abs_sp >= spread_highlight / 2:
                    color = "background-color: #fff4c2"  # medium edge
                else:
                    color = ""  # no highlight
...
# =====================================================
# TAB 5: BET TRACKER (HIT/MISS + LEARNING) WITH PERSISTENCE
# =====================================================
with tab_bets:
    ...
    if not bets:
        ...
        # Ensure bet_category column exists
        ...
        for idx, b in bets_df.iterrows():
            if b.get("bet_category") != "player_prop":
                # leave game bets alone; no auto-grading yet
                continue
        ...
        # sync updated results back to session + disk
        st.session_state["bet_tracker"] = bets_df.to_dict(orient="records")
        save_bets_to_disk(st.session_state["bet_tracker"])

        # ---- Display player props (auto-graded) ----
        ...
        if props_view.empty:
            ...
            for c in show_cols_props:
                if c not in props_view.columns:
                    props_view[c] = None

            if mobile_mode:
                with st.expander("Show player prop bets table"):
                    st.dataframe(props_view[show_cols_props], use_container_width=True)
            else:
                st.dataframe(props_view[show_cols_props], use_container_width=True)

        # ---- Display game bets (ML + spreads) separately ----
        ...
        # ---------- LEARNING FROM MISSES (PLAYER PROPS ONLY) ----------
        st.markdown("### Learning from misses (player props only)")

        hits_df = props_view[props_view["result"].isin(["Hit", "Miss"])].copy()
        if hits_df.empty:
            st.info("No completed player prop bets yet to learn from.")
        else:
            # Hit rate by market
            market_perf = (
