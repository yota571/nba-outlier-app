...
# - NEW: Game Moneylines tab – model win % and ML odds from prop edges
# =====================================================
...
# SIDEBAR UI
# =====================================================
st.sidebar.title("NBA Outlier-Style App (PrizePicks) – v1.5.0")
...
# NEW: ML prediction control
min_props_for_ml = st.sidebar.slider(
    "Min props per team for moneyline prediction", min_value=1, max_value=20, value=3, step=1
)
...
st.sidebar.markdown("---")
with st.sidebar.expander("What is Edge / Confidence / Odds / Moneylines / Bet side?"):
    st.write(
...
# =====================================================
st.title("NBA Prop Edge Finder (PrizePicks) – v1.5.0")
st.caption(
    "PrizePicks full-game props vs nba_api stats → edges, confidence %, prediction, model odds, game moneylines, trends, and bet tracking."
)
...
# =====================================================
# BUILD GAME MONEYLINE PREDICTIONS
# =====================================================
def build_moneyline_predictions(
    ...
    - Aggregate per (game_label, team, opponent)
    - Logistic + normalization per game to get win_prob, then convert to ML odds
    """
    if scoring_markets is None:
        scoring_markets = ["points", "pra", "fs", "threes"]

    if edges.empty or "game_label" not in edges.columns:
        return pd.DataFrame()

    df_ml = edges.copy()
    df_ml = df_ml[df_ml["market"].isin(scoring_markets)]

    if df_ml.empty or edge_col not in df_ml.columns:
        return pd.DataFrame()
    ...
    # normalize within each game so probabilities for both teams add up to ~1
    grp["sum_prob_game"] = grp.groupby("game_label")["raw_prob"].transform("sum")
    ...
    grp = grp.sort_values(["win_prob", "game_label"], ascending=[False, True])
...
tab_cards, tab_table, tab_player, tab_games, tab_bets = st.tabs(
    ["Cards & Explanation", "Table", "Player Detail", "Game Moneylines", "Bet Tracker"]
)
...
# =====================================================
# TAB 4: GAME MONEYLINES (MODEL WIN % & ML ODDS)
# =====================================================
with tab_games:
    st.write("### Game Moneylines (Model)")

    if games_ml_df.empty:
        st.info(
            "Not enough scoring-related props to build game-level moneyline predictions yet.\n\n"
        ...
        if ml_df.empty:
            st.warning(
                f"No teams meet the minimum of {min_props_for_ml} props for a moneyline prediction."
            )
        else:
            show_cols = [
                "game_label",
            ...
            ml_df["win_prob_pct"] = (ml_df["win_prob"] * 100).round(1)

            st.markdown(
                "These are **heuristic model moneylines** built from your prop edges:\n"
            ...
            st.markdown("#### All model moneylines (by team)")
            st.dataframe(
                ml_df[show_cols + ["win_prob_pct"]],
                use_container_width=True,
            )
