...
def load_prizepicks_nba_props() -> pd.DataFrame:
    ...
    - NBA only
    - active/open markets (filter out only clearly closed/settled)
...
def get_team_logo_url(team_name: str | None) -> str | None:
    ...
    This function is very forgiving about the input and will never crash;
    it returns None if it can't find a match.
    """
    if not team_name:
        return None
    ...
    # 4) Partial match inside full_name (e.g. 'los angeles lakers', 'celtics')
    for t in all_teams:
...
# SIDEBAR UI
# =====================================================
st.sidebar.title("NBA Outlier-Style App (PrizePicks) – v2.0.0 + ML")
...
# ML-style team model controls (already heuristic, not true team ML)
...
st.sidebar.markdown("---")
with st.sidebar.expander("What is Edge / Confidence / Odds / ML / Moneylines / Spreads / Bet side?"):
    st.write(
        ...
        "- **Model odds** = fair American odds implied by the historical data.\n"
        "- **ML Over prob** = probability of Over from your trained ML model (over_model.pkl).\n"
        "- **Bet side** = Over / Under / No clear edge, based on predicted score vs the line.\n"
        "- **Game Moneylines tab** = uses prop edges to build a team-level signal, "
        "then converts that to win probability and moneyline-style odds.\n"
        "- **Model spreads** = same team-level signal, centered within each game and scaled into points.\n"
...
if over_model is None:
    st.sidebar.warning(
        "ML model file 'over_model.pkl' not found or could not be loaded.\n\n"
        "The app will still run using historical stats only; ML fields will be empty.\n"
        "Train and save your model as over_model.pkl to enable ML probabilities."
    )
...
# =====================================================
st.title("NBA Prop Edge Finder (PrizePicks) – v2.0.0 + ML")
st.caption(
    "PrizePicks full-game props vs nba_api stats → edges, confidence %, prediction, model odds, "
    "ML Over probabilities, game moneylines & spreads, trends, and bet tracking with learning."
)
...
status_text.text("Computing edges, predictions, confidence, bet side & ML probabilities…")
...
def build_ml_features(
    ...
    """
    Features used for ML. IMPORTANT:
    Your training script must create these same columns when training over_model.pkl.
    """
    return {
...
for _, row in df.iterrows():
    ...
    try:
        ...
        # if ML fails for any reason, just leave ML fields as None
        pass
...
def build_moneyline_predictions(edges, scoring_markets=None):
    """
    Build heuristic game moneyline + spread predictions using prop edges.
    ...
    - Model spread = centered signal * spread_factor (points)
    """
    ...
    spread_factor = 7.0  # knob: how many points per unit of centered signal
...
tab_cards, tab_table, tab_player, tab_games, tab_bets = st.tabs(
    ["Cards & Explanation", "Table", "Player Detail", "Game Moneylines & Spreads", "Bet Tracker"]
)
...
with tab_cards:
    ...
    if top_n == 0:
        ...
        def render_player_card(r, k_suffix: str):
            ...
            with card:
                ...
                # --- Track this bet button (player prop) ---
                if st.button(
                    "Track this bet",
    ...
    st.write("### Prop Explanation (Why this prediction, confidence, odds & ML)")
    ...
    if "explain_row" in st.session_state:
        ...
        if row_matches.empty:
            ...
            with col_right:
                ...
                st.write(f"- Confidence (historical): **{confidence_pct:.1f}%**")
                ...
                if ml_prob is not None and ml_odds is not None:
                    ...
                    st.write("- ML Over probability: _model not available or no ML output_")
                st.write(f"- Recommended side (historical): **{bet_side}**  (odds: **{bet_odds}**)")

                st.markdown("**How the signals are calculated:**")
                st.write(
                    "- **Historical**: Hit rate over the last N games (how often they went over the line) "
                    "and edge (average above the line).\n"
                    "- **Confidence %**: `0.6 × HitRate + 0.4 × EdgeStrength`.\n"
                    "- **Historical odds**: convert hit-rate to fair American odds.\n"
                    "- **ML Over prob**: model learns from many past games + features, "
                    "then outputs `P(Over)` which we convert to American odds."
                )
...
# =====================================================
# TAB 2: FULL EDGES TABLE (with player filter + ML columns)
# =====================================================
with tab_table:
    ...
    if selected_table_player != "All players":
        ...
        if not player_all_lines.empty:
            ...
            if mobile_mode:
                with st.expander("Show player lines table"):
                    st.dataframe(
...
with tab_games:
    ...
    if games_ml_df.empty:
        ...
        if ml_df.empty:
            ...
            st.markdown(
                "These are **heuristic model moneylines and spreads** built from your prop edges:\n"
                "- Uses mainly points / PRA / threes / fantasy score props\n"
                "- Signal = edge × confidence × direction (Over vs Under)\n"
                "- Aggregated per team and normalized within each game\n"
                "- Converted to win probability and American ML odds\n"
                "- Model spread = centered signal × scaling factor (points)\n"
                "  (Positive ⇒ model favorite by that many points.)"
            )
                ...
                def render_game_card(fav, k_suffix: str):
                    ...
                    with card:
                        ...
                        with top_c1:
                            ...
                            st.markdown(
                                f"- Model spread: **{fav['model_spread']}**"
                            )
                            st.markdown(
                        ...
                        with top_c2:
                            if dog:
                                st.markdown("**Underdog (model)**")
                                st.markdown(f"**Team:** {dog['team']}")
                                st.markdown(
                                    f"- Win %: **{dog['win_prob_pct']:.1f}%**  "
                                    f"({dog['ml_odds']})"
                                )
                                st.markdown(
                                    f"- Model spread: **{dog['model_spread']}**"
                                )
                                st.markdown(
                        ...
                        wp = fav["win_prob_pct"]
                        sp = fav["model_spread"]
                        label = f"{fav_team} {sp:+.1f} (≈ {wp:.1f}% ML)"
                        st.markdown(f"**Model lean:** {label}")
                        st.progress(min(max(wp / 100.0, 0.0), 1.0))
                        ...
                        with col_btn1:
                            if st.button(
                                ...
                                bet = {
                                    ...
                                    "bet_odds": fav["ml_odds"],
                                    "predicted_score": None,
                                    "confidence_pct": fav["avg_confidence"],
                                    "game_time": None,
                                    "game_key": game_key,
                                    "model_spread": fav["model_spread"],
                                    "win_prob_pct": fav["win_prob_pct"],
                                }
                                st.session_state["bet_tracker"].append(bet)
                                save_bets_to_disk(st.session_state["bet_tracker"])
                                st.success("Game ML added to tracker.")

                        with col_btn2:
                            if st.button(
                                "Track favorite spread",
                                ...
                                st.session_state["bet_tracker"].append(bet)
                                save_bets_to_disk(st.session_state["bet_tracker"])
                                st.success("Game spread added to tracker.")
            ...
            st.markdown("#### Model Moneylines (favorites per game)")
            ...
            def highlight_ml(row):
                styles = [""] * len(row)
                try:
                    wp = float(row["win_prob_pct"])
                    props_c = float(row["props_count"])
                except Exception:
                    return styles

                if wp >= ml_winprob_highlight and props_c >= min_props_for_ml:
                    color = "background-color: #d4f8d4"
                elif wp < 50:
                    color = "background-color: #f8d4d4"
                else:
            ...
            styled_ml = (
                ...
                .style.apply(highlight_ml, axis=1)
            )

            if mobile_mode:
                with st.expander("Show model moneylines table"):
                    st.dataframe(styled_ml, use_container_width=True)
            else:
                st.dataframe(styled_ml, use_container_width=True)

            st.markdown(
                f"#### Model Spreads (favorites per game) – highlight |spread| ≥ {spread_highlight}"
            )
            ...
            def highlight_spread(row):
                ...
                if abs_sp >= spread_highlight and props_c >= min_props_for_ml:
                    color = "background-color: #d4f8d4"
                elif abs_sp >= spread_highlight / 2:
                    color = "background-color: #fff4c2"
                else:
                    color = ""
            ...
            styled_spreads = (
                ...
                .sort_values("model_spread", key=lambda s: s.abs(), ascending=False)
                .style.apply(highlight_spread, axis=1)
            )

            if mobile_mode:
                with st.expander("Show model spreads table"):
    ...
    if not bets:
        ...
        if not props_view.empty:
            ...
            OPEN_STATUSES = {
                ...
                "No game_time",
                "No game log",
                "Not played / not final yet",
                "Unknown",
            }

            def is_open_result(val):
                if pd.isna(val):
                    return True
                s = str(val)
                return s in OPEN_STATUSES

            open_mask = props_view["result"].apply(is_open_result)
            open_props = props_view[open_mask].copy()
            completed_props_for_table = props_view[~open_mask].copy()
            ...
            show_cols_completed = show_cols_base + ["predicted_score", "actual_stat"]

            for c in show_cols_completed:
                if c not in completed_props_for_table.columns:
                    completed_props_for_table[c] = None
            for c in show_cols_base:
                if c not in open_props.columns:
                    open_props[c] = None

            st.markdown("**Open / pending player props**")
            if open_props.empty:
                st.caption("_No open player props – everything graded._")
            else:
                if mobile_mode:
                    with st.expander("Show open player props"):
                        st.dataframe(open_props[show_cols_base], use_container_width=True)
                else:
                    st.dataframe(open_props[show_cols_base], use_container_width=True)

            st.markdown("**Completed player props (Hit / Miss / N/A)**")

            if completed_props_for_table.empty:
                st.caption("_No completed player props yet._")
            else:
                ...
                styled_completed = (
                    completed_props_for_table[show_cols_completed]
                    .sort_values("game_time", ascending=False)
                    .style.apply(highlight_results, axis=1)
                )

                if mobile_mode:
                    with st.expander("Show completed player props"):
                        st.dataframe(styled_completed, use_container_width=True)
                else:
                    st.dataframe(styled_completed, use_container_width=True)

        st.markdown("#### Game bets (ML & spreads)")

        if game_view.empty:
            st.info("No game (ML/spread) bets tracked yet.")
        else:
            show_cols_game = [
                "bet_category",
                "game_key",
