...
import plotly.graph_objects as go  # <-- NEW: for What-If bar chart
                ...
                if row_match.empty:
                    ...
                    if series.empty:
                        ...
                        if last_n.empty:
                            ...
                            # ============================
                            # NEW: BAR CHART VS YOUR LINE
                            # ============================
                            last_n_vals = last_n.copy()
                            game_dates = glog["GAME_DATE"].iloc[:games_to_look_back]

                            # Format dates nicely for x-axis
                            x_labels = game_dates.dt.strftime("%m-%d").tolist()

                            values = last_n_vals.values
                            colors = [
                                "#00C853" if v > new_line else "#D50000" if v < new_line else "#B0BEC5"
                                for v in values
                            ]
                            ...
                            fig.add_bar(
                                ...
                                y=values,
                                marker_color=colors,
                                name=sel_market.upper(),
                            )

                            # Horizontal dashed line at your chosen line
                            fig.add_hline(
                                y=new_line,
                                line_dash="dash",
                                line_color="#FFC107",
                                annotation_text=f"Line {new_line}",
                                annotation_position="top left",
                            )

                            fig.update_layout(
                                title=f"Last {games_to_look_back} games vs your line ({new_line})",
                                xaxis_title="Game date",
                                yaxis_title=sel_market.upper(),
